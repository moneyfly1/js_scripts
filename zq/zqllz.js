...
var nowDay = new Date();
var cacheKeyName = `cache_zqllz_reward_${nowDay.Format("yyyyMMdd")}`;
var failureCacheKeyName = `cache_zqllz_reward_failure_${nowDay.Format("yyyyMMdd")}`;
...
if (zqllzbody) {
    if (zqllzbody.indexOf("&") == -1) {
        ...
        for (let i = 0; i < zqllzbodys.length; i++) {
            zqllzbodys[i] = zqllzbodys[i].replace("zqkd_param=", "p=");
        }
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        for (let k = 0; k < zqllzbodyArr.length; k++) {
            ...
            if (zqllzbody1.indexOf("zqkd_param=") >= -1) {
                zqllzbody1 = zqllzbody1.replace("zqkd_param=", "p=")
            }
            console.log(`--------第 ${k + 1} 次浏览赚激活执行中--------\n`)
            let tmpCache = containCache(cacheKeyName, zqllzbody1);
            if (tmpCache !== true) {
                await Start()
                await $.wait(1000);
            } else {
                console.log(`--------第 ${k + 1} 次浏览赚已执行成功【cache】--------\n`)
            }
            console.log("\n\n")
        }
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if (result.success === true) {
                    ...
                    let cacheValue = "requestBody:" + zqllzbody1 + ",response:" + data;
                    setCache(failureCacheKeyName, cacheValue);
                    //setCache(cacheKeyName,tmpBody);
                    console.log(data);
                    console.log('\n激活浏览赚任务失败')
                }
            } catch (e) {
                console.log("function Start() Response Error");
                console.log(e);
            } finally {
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if (result.items.score !== "undefined") {
                    let setCacheResult = setCache(cacheKeyName, zqllzbody1);
                    console.log("setCacheResult:" + setCacheResult);
                    console.log('\n浏览赚获得：' + result.items.score + '金币')
                } else {
                    console.log('\n领取奖励失败')
                }
            } catch (e) {
                console.log("function end() Response Error:");
                console.log(e);
            } finally {
                resolve()
            }
